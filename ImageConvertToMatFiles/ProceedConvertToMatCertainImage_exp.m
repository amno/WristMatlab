...
%                                         subplot(1,2,2),imagesc(regionBelowLunate_nan),colormap bone, title('region below lunate NaN');
                    ...
                    %Xray_maskP                  = regionprops(maskDicom,readDicom,'Area','Centroid','meanIntensity');
                    
                    %subplot the results
%                     figure, 
%                     subplot(2,4,1), imagesc(readDicom), colormap bone, title('Original Image');
%                     subplot(2,4,2), imagesc(dicomRotate), colormap bone, title('Aligned Image');
%                     subplot (2,4,3),imagesc(dicomRotRemoveEdge),colormap bone, title('edge removed');
%                     subplot(2,4,4),imagesc(Cr),colormap bone, title('Cropped Image');
%                     subplot(2,4,5),imshow(EdgeIm), title('Edge of Cropped Image');
%                     subplot(2,4,6),imshow(maskDicom),title('Mask Image');
%                     subplot(2,4,7),imagesc(mixedImg),colormap bone, title(' Mask and Cropped Image merged'); 
%                     subplot(2,4,8),plot(mean(dicomRotRemoveEdge,2)), title ('EdgeRemovedBoneIntensity')
...
%                     if ChooseDir ==0
%                         currentData=load(strcat(saveDir,'*_N_*.mat'));
%                     else
%                         currentData = load(strcat(saveDir,'*_P_*.mat'));
%                     end
                end
            end
        end
